from ts_benchmark.baselines.NPMoETSF.layers.linear_extractor_cluster import Linear_extractor_cluster
import torch.nn as nn
from einops import rearrange
from ts_benchmark.baselines.NPMoETSF.utils.masked_attention import Mahalanobis_mask, Wavelet_Mahalanobis_mask, Encoder, \
    EncoderLayer, FullAttention, AttentionLayer
import torch


class NPMoETSFModel(nn.Module):
    def __init__(self, config):
        super(NPMoETSFModel, self).__init__()
        self.cluster = Linear_extractor_cluster(config)
        self.CI = config.CI
        self.n_vars = config.enc_in

        self.mask_generator = Wavelet_Mahalanobis_mask(config.seq_len, levels=3)
